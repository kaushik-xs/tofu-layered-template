...
variable "instances" {
  description = <<-EOT
    ...
    Optional: vpc_name / network_name (tags), private_ip (static address in subnet), ami_id, instance_type,
    security_group_ids (defaults to VPC default SG if empty), user_data, tags.
  EOT
  type        = map(any)
}
