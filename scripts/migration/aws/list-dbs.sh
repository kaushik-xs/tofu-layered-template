...
# on each database, from a Docker container running on an AWS EC2 instance.
#
# Usage (run this on your LOCAL machine):
#   ./scripts/migration/aws/list-dbs.sh
#
# Connection types:
...
#   4.  Verifies connectivity to the instance
...
CONF_FILE="${CONF_DIR}/aws-create-app-db.conf"   # reuse same config as create-app-db.sh
...
echo "It connects to an AWS EC2 instance to list databases and users."
