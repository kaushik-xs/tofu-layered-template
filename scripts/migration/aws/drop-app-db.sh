...
# container running on an AWS EC2 instance.
#
# Usage (run this on your LOCAL machine):
#   ./scripts/migration/aws/drop-app-db.sh
#
# Connection types:
...
#
# SQL is piped via stdin (docker exec -i) to avoid quoting issues over the tunnel.
#
set -euo pipefail

# ── Config file ───────────────────────────────────────────────────────────────
CONF_DIR="${HOME}/.config/pg-migration"
CONF_FILE="${CONF_DIR}/aws-drop-app-db.conf"
...
save_config() {
  mkdir -p "${CONF_DIR}"
  cat > "${CONF_FILE}" << CONF
# pg-migration aws drop-app-db — last used values (auto-generated, do not commit)
...
echo "It connects to an AWS EC2 instance to drop the database and user."
